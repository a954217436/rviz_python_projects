...
class KalmanBoxTracker(object):
    ...
    def __init__(self, bbox, gamma=0.5):
        """
    ...
    def update(self, bbox):
        ...
        Updates the state vector with observed bbox.
            bbox : (cx, cy, cz, dx, dy, dz, heading, score, class, timestamp)  
        """
        if self.id == 1:
            print("bbox[:2]              : ", bbox[:2])
            # print("self.last_box         : ", self.last_box[:2])
            # if self.last_box_filtered is not None:
            #     print("self.last_box_filtered: ", self.last_box_filtered[:2])
        if bbox[-1] - self.last_box[-1] < 0.01:
            velo_mesured = np.array([0., 0.])
        else:
            # if self.last_box_filtered is not None:
            #     velo_mesured = (bbox[:2] - self.last_box_filtered[:2]) / (bbox[-1] - self.last_box[-1])
            # else:
            velo_mesured = (bbox[:2] - self.last_box[:2]) / (bbox[-1] - self.last_box[-1])
        self.last_box= bbox
        self.mesured_velo = velo_mesured
        if self.id == 1:
            print(velo_mesured)
            print("velo_mesured: ", np.linalg.norm(velo_mesured, 2) * 3.6)
        self.last_timestamp = bbox[-1]

        # state_mesured = np.hstack((bbox[:7], velo_mesured))
        # print("state_mesured: ", state_mesured)
        # self.kf.update(state_mesured)
        self.kf.update(bbox[:7])
        
        self.last_box_filtered = self.kf.x.reshape(1, -1)[0]
        self.history.append(self.last_box_filtered)
...
class Sort(object):
    ...
    def update(self, dets=np.empty((0, 10))):
        """
        Params:
            dets - a numpy array of detections in the format [[cx, cy, cz, dx, dy, dz, heading, score, class, timestamp],
        ...
        matched, unmatched_dets, unmatched_trks = \
            associate_detections_to_trackers(dets, predict_states, self.iou_threshold)
