...
class MOTModelTwoStage(object):
    def __init__(self, iou_threshold=0.3, high_thres=0.5, low_thres=0.1, use_giou=False):
        """
    ...
    def update(self, dets=None):
        ...
        matched, unmatched_dets, unmatched_trks = \
            associate_detections_to_trackers(det_boxes_1st, predict_states, self.iou_threshold, use_giou=self.use_giou)
        ...
        if len(remained_trks) > 0 and len(det_boxes_2nd) > 0:
            ...
            r_matched, r_unmatched_dets, r_unmatched_trks = \
                associate_detections_to_trackers(det_boxes_2nd, r_predict_states, self.iou_threshold, use_giou=self.use_giou)
